# Minimal I/O for the Stop hook: stdin parsing, raw archive, per-event
# log, and the Stop-hook decision envelopes (continue / block).
# Fail-open by default: any error allows the stop so a broken hook
# can't trap the user at session end.
...
def parse_stdin(hook_script: str) -> HookInvocation:
    ...
    # trap the user at session end. Empty raw dict propagates downstream.
    raw: dict[str, Any] = {}
    if payload:
    ...
    inv = HookInvocation(
        event=str(raw.get("hook_event_name") or "Stop"),
        cwd=Path(cwd_raw),
...
def log(inv: HookInvocation, msg: str, *, level: str = "info") -> None:
    ...
    with contextlib.suppress(OSError):
        print(f"[stop-check:{inv.event}] {msg}", file=sys.stderr)
    with contextlib.suppress(OSError):
...
def emit_continue_with_message(message: str | None = None) -> int:
    """Allow the session to stop. If `message` is set, surface it as a
...
def emit_block(reason: str) -> int:
    """Block the stop with a reason. The host CLI surfaces the reason
    to the model so it can address the issue (e.g. commit, push) and
    re-attempt the stop later — at which point stop_hook_active=true
    and we let it through."""
    _write_stdout({"decision": "block", "reason": reason})
    return 0
